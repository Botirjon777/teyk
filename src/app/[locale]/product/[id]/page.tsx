...
import { IoArrowBack, IoHeartOutline, IoHeart, IoAdd, IoRemove } from "react-icons/io5";
...
const getProductById = (id: string) => {
    const products = {
        ...
        "2": {
            id: "2",
            name: "Iced Americano",
            description: "Bold espresso shots with cold water and ice for a refreshing taste",
            price: 18000,
...
export default function ProductPage({ params }: ProductPageProps) {
    ...
    const [milk, setMilk] = useState<'no-milk' | 'regular-milk' | 'oat-milk' | 'almond-milk' | 'soy-milk'>('regular-milk');
        ...
        <div className="page-with-nav bg-background">
            ...
            <div className="sticky top-0 z-40 bg-card-background border-b border-border shadow-shadow-sm">
                <div className="flex items-center justify-between px-4 py-3">
                    ...
                    <h1 className="text-lg font-bold text-primary-text">Product Details</h1>
                    <button
            ...
            <div className="page container">
                ...
                {/* Product Info */}
                <div className="mb-6">
                    <h2 className="text-2xl font-bold text-primary-text mb-2">{product.name}</h2>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-secondary-text">{product.category}</span>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {sizeOptions.map((option) => (
                            <button
                                ...
                                className={`p-3 rounded-lg border-2 transition-all ${size === option.value
                            ...
                            >
                                ...
                                {option.price > 0 && (
                                    <div className="text-xs text-secondary-text">+{option.price.toLocaleString()} UZS</div>
                                )}
                            </button>
                        ))}
                    </div>
                ...
                {/* Temperature */}
                <div className="mb-6">
                    <h3 className="text-lg font-semibold text-primary-text mb-3">Temperature</h3>
                    <div className="grid grid-cols-2 gap-3">
                        {temperatureOptions.map((option) => (
                            <button
                                key={option.value}
                                onClick={() => setTemperature(option.value as typeof temperature)}
                                className={`p-3 rounded-lg border-2 transition-all ${temperature === option.value
                                    ? 'border-primary-green bg-soft-teal text-primary-green font-semibold'
                                    : 'border-border bg-card-background text-primary-text hover:border-secondary-teal'
                                    }`}
                            >
                ...
                {/* Sugar Level */}
                <div className="mb-6">
                    <h3 className="text-lg font-semibold text-primary-text mb-3">Sugar Level</h3>
                    <div className="grid grid-cols-2 gap-3">
                        {sugarOptions.map((option) => (
                            <button
                                key={option.value}
                                onClick={() => setSugar(option.value as typeof sugar)}
                                className={`p-3 rounded-lg border-2 transition-all ${sugar === option.value
                                    ? 'border-primary-green bg-soft-teal text-primary-green font-semibold'
                                    : 'border-border bg-card-background text-primary-text hover:border-secondary-teal'
                                    }`}
                            >
                ...
                {/* Milk Type */}
                <div className="mb-6">
                    <h3 className="text-lg font-semibold text-primary-text mb-3">Milk Type</h3>
                    <div className="grid grid-cols-2 gap-3">
                        {milkOptions.map((option) => (
                            <button
                                key={option.value}
                                onClick={() => setMilk(option.value as typeof milk)}
                                className={`p-3 rounded-lg border-2 transition-all ${milk === option.value
                                    ? 'border-primary-green bg-soft-teal text-primary-green font-semibold'
                                    : 'border-border bg-card-background text-primary-text hover:border-secondary-teal'
                                    }`}
                            >
                ...
                {/* Extras */}
                <div className="mb-6">
                    <h3 className="text-lg font-semibold text-primary-text mb-3">Add Extras</h3>
                    <div className="grid grid-cols-2 gap-3">
                        {extrasOptions.map((option) => (
                            <button
                                key={option.value}
                                onClick={() => toggleExtra(option.value)}
                                className={`p-3 rounded-lg border-2 transition-all ${extras.includes(option.value)
                                    ? 'border-primary-green bg-soft-teal text-primary-green font-semibold'
                                    : 'border-border bg-card-background text-primary-text hover:border-secondary-teal'
                                    }`}
                            >
                ...
                {/* Quantity */}
                <div className="mb-6">
                    <h3 className="text-lg font-semibold text-primary-text mb-3">Quantity</h3>
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-xl font-bold text-primary-text min-w-[3rem] text-center">
                            {quantity}
                        </span>
                        <button
            ...
            {/* Fixed Bottom Bar */}
            <div className="fixed bottom-0 left-0 right-0 bg-card-background border-t border-border shadow-shadow-lg p-4 z-40">
